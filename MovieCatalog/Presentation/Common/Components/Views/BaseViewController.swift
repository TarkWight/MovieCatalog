...
class BaseViewController: UIViewController {
    ...
    func setBackgroundImage(named imageName: String, topOffset: CGFloat, heightMultiplier: CGFloat, cornerRadius: CGFloat = 32) {
            if let image = UIImage(named: imageName) {
                ...
                maskLayer.path = UIBezierPath(roundedRect: CGRect(x: 0, y: 0, width: view.frame.width, height: view.frame.height * heightMultiplier),
                                              byRoundingCorners: [.bottomLeft, .bottomRight],
                                              cornerRadii: CGSize(width: cornerRadius, height: cornerRadius)).cgPath
                imageView.layer.mask = maskLayer

                applyFadeOutGradient(to: imageView, heightMultiplier: heightMultiplier)
    ...
    private func applyFadeOutGradient(to imageView: UIImageView, heightMultiplier: CGFloat) {
        ...
        topGradientLayer.endPoint = CGPoint(x: 0.5, y: 0.2)
        topGradientLayer.frame = CGRect(x: 0, y: 0, width: view.frame.width, height: 100)
        ...
        bottomGradientLayer.startPoint = CGPoint(x: 0.5, y: 0.8)
        bottomGradientLayer.endPoint = CGPoint(x: 0.5, y: 1.0)
        bottomGradientLayer.frame = CGRect(x: 0, y: imageView.frame.height - 100, width: view.frame.width, height: 100)
